...
**核心原则**：在添加新功能前，必须先理解现有架构，能复用就复用，包括组件、Hooks、工具函数。

#### 第一步：理解现有架构

1. **先看文档**：检查 `docs/` 目录是否有相关功能的说明文档

2. **看实际代码**：文档可能滞后，还需要看实际的：
   - `src/components/` - 现有组件
   - `src/hooks/` - 现有 Hooks
   - `src/utils/` - 工具函数

3. **用自然语言总结**：理解清楚后，先用自然语言描述：
   - 这个功能需要什么 UI 组件？
   - 现有哪些组件/Hooks 可能已经覆盖了这个需求？

#### 第二步：决策 - 复用 vs 新建

| 场景 | 决策 |
|------|------|
| 现有组件已有类似功能 | ✅ 扩展现有组件，不新建 |
| 现有 Hook 逻辑相似 | ✅ 扩展现有 Hook，不新建 |
| 确实是全新领域，无法复用 | ⚠️ 新建，但先写文档说明为什么不能复用 |
