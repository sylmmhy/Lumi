...
| **云端 Supabase 优先** | 直接使用 MCP 工具操作云端 Supabase，可直接部署 |
...
| **前端** | [sylmmhy/Lumi](https://github.com/sylmmhy/Lumi.git) | `../Lumi` | React + TypeScript + Vite |

> **历史背景**：两个仓库原本是同一个 repo，为了 hackathon 方便进行了分 repo 处理。部分配置和规则在两个仓库中相互引用。
>
> **本地目录约定**：两个仓库应放在同一父目录下，即 `Lumi/` 和 `Lumi-supabase/` 是兄弟目录。
