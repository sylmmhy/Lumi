...
所有 commit 必须添加 `sylmmhy` 作为 coauthor，以确保 Vercel 能够识别并触发自动部署。
...
Co-Authored-By: sylmmhy <sylmmhy@users.noreply.github.com>
EOF
)"
```
...
```bash
git commit -m "$(cat <<'EOF'
fix: 修复 DevConsole 在 iOS WebView 中的剪贴板复制问题
...
**为什么需要 coauthor？**
- Vercel 项目关联的是 `sylmmhy` 的 GitHub 账号
- 只有当 commit 的作者或 coauthor 包含 `sylmmhy` 时，Vercel 才会触发自动部署
- 这确保了每次推送后，meetlumi.org 会自动更新

**部署流程**：
```bash
# 1. Commit（已包含 coauthor）
git commit -m "..."
