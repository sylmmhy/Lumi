...
### Bug 修复记录
- ✅ **#1**: 老用户本地缓存未设置问题（第十三节）
- ✅ **#2**: RLS 问题，改用 accessToken 查询（第十三节 13.5）
- ✅ **#3**: 用户名被重置问题（第十四节）
...
| 2026-01-14 | **网页端优化完成** | 修改 App.tsx 和 HabitOnboardingPage.tsx |
...
**问题**：上述方案实施后，数据库查询返回空数组 `[]`，无法获取用户的 onboarding 状态。

**日志表现**：
```
Could not parse onboarding status (empty or invalid array), returning false
```

**根本原因**：
- iOS 端调用 `fetchHabitOnboardingStatus()` 时，使用的是 `apiKey`（公开的匿名密钥）
- Supabase 的 RLS（Row Level Security）策略要求使用用户的 `accessToken` 才能查询到数据
- 用 `apiKey` 查询相当于"匿名用户"，RLS 会拒绝返回任何数据

**解决方案**：

修改 `SupabaseClient.swift` 的 `fetchHabitOnboardingStatus()` 方法，接收并使用用户的 `accessToken`：

```swift
/// 从数据库获取用户的 habit onboarding 完成状态
func fetchHabitOnboardingStatus(userId: String, accessToken: String? = nil) async throws -> Bool {
    // ... URL 构建 ...

    // apikey header 是必须的（用于标识项目）
    request.setValue(apiKey, forHTTPHeaderField: "apikey")

    // 如果有 accessToken，使用它来授权（绕过 RLS）；否则使用 apiKey
...
修改 `AppCoordinator.swift`，在调用时传入 `accessToken`：

```swift
// 获取 accessToken 用于绕过 RLS
let accessToken = await KeychainManager.shared.getAccessTokenAsync()

// 查询数据库时使用 accessToken
...
- 预期行为：App 启动后，直接显示主页（查询数据库后决定）

---

## 十四、Bug 修复：重新登录后用户名被重置问题

### 14.1 问题描述

**现象**：用户设置过自定义显示名（如 "张三"），但重新登录后，名字被重置为 OAuth 提供商返回的原始名字（如 Google 账号的 "John Doe"）。

**日志表现**：
```
[恢复会话] userName 更新: 张三 → John Doe
```

### 14.2 问题根源

在 `AuthContext.tsx` 中，多个位置在设置 `userName` 时，优先使用了 OAuth 返回的 `user_metadata.full_name`：
...
这导致每次重新登录时，即使用户已经设置过自定义名字（保存在 `localStorage.user_name`），也会被 OAuth 的名字覆盖。
...
| 行号（约） | 函数/位置 | 修改内容 |
|-----------|----------|----------|
| ~293 | validateSessionWithSupabase | 修复 userName 优先级 |
| ~411 | session restore | 修复 userName 优先级 |
| ~827-832 | 密码登录成功 | 修复 userName 优先级 |
| ~1104-1109 | OTP 登录成功 | 修复 userName 优先级 |
| ~1505-1512 | 原生 App 登录 | 修复 userName 优先级 |

### 14.5 修改原则

- `localStorage.user_name`：用户**主动设置**的自定义名字，应该**最高优先级**
- `user_metadata.full_name`：OAuth 提供商（Google/Apple）返回的原始名字，作为**备选**
- 如果两者都没有，则为 `null`
