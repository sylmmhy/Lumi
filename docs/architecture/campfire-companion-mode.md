...
| 功能 | 触发条件 | 效果 |
|------|---------|------|
| **智能空闲断开** | 用户 2 分钟没说话 | 后台静默断开 AI 连接，省 Token |
| **静默模式** | 主动：用户说"别说话" / 被动：2分钟不说话 | AI 停止主动说话，安静陪伴 |

**组合效果**：
1. 用户说"我要专心了" → AI 回应后进入静默模式（不主动说话）
2. 或：用户安静工作 2 分钟 → 自动进入静默模式并断开连接（省 Token）
...
用户    开始工作      和AI聊了几句   专心工作中     突然想说话
        ...
        │            │            │            │
        ▼            ▼            ▼            ▼
AI状态  [==连接中===] [2分钟不说话] [静默断开]    [秒级重连]
                      自动断开      用户不知道     用户不知道

        │            │            │            │
        ▼            ▼            ▼            ▼
用户    "开始专注吧"  （安静工作）   （继续工作）   "我刚才..."
感受    AI回应                                   AI立刻回应！
                                                还记得之前聊的
```
...
**开发测试入口**：
- 在 `https://localhost:5173/dev` 页面添加「进入静默模式」按钮，方便测试主动触发
...
**关键特点**：
- 用户**不会看到**任何"进入静默模式"的提示
- 篝火声音和动画**继续播放**，保持陪伴氛围
- 这是**省 Token 的核心机制**

#### 共同设计原则

- AI 主动触发时用温暖简短的话回应（不是冷冰冰的"好的"）
- 用户说任何话都会自动解除静默模式
...
**验收标准**：
- [x] 用户说"别说话"等相关话语后，AI 进入静默模式
- [x] 静默模式下虚拟消息停止发送
- [x] 用户主动说话后自动恢复正常模式
- [x] AI 的静默回应自然友好（不是冷冰冰的）
...
> 备注：雨声/咖啡厅环境音目前仅提供占位入口，待提供真实音源后补齐。
...
2. 如果超过 2 分钟没有用户语音，调用 `geminiLive.disconnect()`
