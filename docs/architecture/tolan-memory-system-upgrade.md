# Tolan 级别记忆系统升级方案

> 文档版本: 1.0
> 创建日期: 2026-01-27
> 状态: 待实现

---

## 目录

1. [概述](#1-概述)
2. [功能详解（通俗版）](#2-功能详解通俗版)
3. [系统架构](#3-系统架构)
4. [数据库设计](#4-数据库设计)
5. [Edge Function 实现](#5-edge-function-实现)
6. [性能优化](#6-性能优化)
7. [部署配置](#7-部署配置)
8. [验证计划](#8-验证计划)
9. [回滚策略](#9-回滚策略)
10. [附录：完整代码](#10-附录完整代码)
11. [常见问题与踩坑记录](#11-常见问题与踩坑记录)
12. [与动态虚拟消息系统集成](#12-与动态虚拟消息系统集成)

---

## 1. 概述

### 1.1 项目背景

当前记忆系统使用简单的文本搜索（textSearch），存在以下问题：
- 只能按关键词匹配，无法理解语义
- 单次搜索可能遗漏相关记忆
- 没有自动清理机制，记忆会越来越乱
- 无法处理矛盾的记忆

### 1.2 升级目标

参考 Tolan AI 的实现，升级为 Multi-Query RAG 架构：

| 功能 | 当前状态 | 升级后 |
|------|---------|--------|
| 问题合成 | ❌ 没有 | ✅ LLM 生成 3-5 个检索问题 |
| 向量搜索 | ❌ 只用文本搜索 | ✅ 多次并行向量搜索 |
| 结果排序 | ❌ 简单排序 | ✅ MRR 智能融合排序 |
| 记忆清理 | ❌ 手动触发 | ✅ 每日自动清理 |
| 矛盾处理 | ❌ 没有 | ✅ 自动检测并解决 |

### 1.3 预期效果

- **检索准确率提升**：从关键词匹配升级为语义理解
- **响应延迟**：< 700ms（用户几乎无感知）
- **记忆质量**：自动保持最新、无矛盾、无冗余

---

## 2. 功能详解（通俗版）

> **五大功能概览**
> | 功能 | 作用 | 触发时机 |
> |------|------|---------|
> | 2.1 问题合成 | 从用户输入生成多个检索问题 | 每次任务开始 |
> | 2.2 多次向量搜索 | 并行搜索，找到更多相关记忆 | 每次任务开始 |
> | 2.3 MRR 智能排序 | 融合多次搜索结果，排出最相关的 | 每次任务开始 |
> | 2.4 **记忆分层检索** | 热/温/冷分层，优先搜热层 | 每次任务开始 |
> | 2.5 夜间自动清理 | 删除低价值、解决矛盾、合并重复 | 每天凌晨 3 点 |

### 2.1 功能一：问题合成（Question Synthesis）

#### 是什么？

让 AI 帮你"想问题"。用户说一句话，AI 自动想出 4-5 个相关的搜索问题。

#### 举个例子

```
用户说：「我周末要去旅行好兴奋」

AI 自动生成的搜索问题：
1. 用户之前去过哪些地方旅行？
2. 用户喜欢什么类型的旅行活动？
3. 用户旅行前通常有什么感受或焦虑？
4. 用户最近提到过什么旅行计划？
5. 用户通常和谁一起旅行？
```

#### 为什么需要？

| 场景 | 没有问题合成 | 有问题合成 |
|------|-------------|-----------|
| 用户说"我周末要去旅行" | 只搜索包含"旅行"的记忆 | 搜索旅行、活动、计划、同伴等多个维度 |
| 找到的记忆 | 可能只有 1-2 条 | 可能找到 5-10 条相关记忆 |
| AI 回复质量 | 泛泛而谈 | 个性化、有细节 |

#### 用户体验对比

**升级前：**
> 用户：我周末要去旅行好兴奋
> AI：听起来很棒！祝你旅途愉快！

**升级后：**
> 用户：我周末要去旅行好兴奋
> AI：太棒了！你上次和 Steven 去 Yosemite 露营玩得很开心，这次也是户外活动吗？我记得你说过喜欢自驾游。

---

### 2.2 功能二：多次向量搜索（Multi-Query RAG）

#### 是什么？

用上面生成的 4-5 个问题，同时进行 4-5 次搜索，找出所有相关的记忆。

#### 怎么搜索？

**第一步：把文字变成"数字指纹"（向量）**

每句话都可以变成一串数字，意思相近的话数字也相近：

```
"用户喜欢户外活动" → [0.23, 0.87, 0.45, 0.12, ...] (1536个数字)
"用户爱爬山露营"   → [0.25, 0.85, 0.47, 0.14, ...] (很接近！)
"用户讨厌运动"     → [0.91, 0.12, 0.33, 0.88, ...] (差很远！)
```

**第二步：计算相似度**

比较两串数字有多接近，用"余弦相似度"：
- 1.0 = 完全一样
- 0.8+ = 非常相似
- 0.5 = 有点关系
- 0.0 = 完全无关

**第三步：找出最相似的记忆**

```
搜索问题: "用户喜欢什么户外活动"
        ↓
数据库中的记忆：
- "用户和朋友去 Yosemite 露营" → 相似度 0.89 ✅
- "用户喜欢早上慢跑" → 相似度 0.72 ✅
- "用户讨厌早起" → 相似度 0.31 ❌
        ↓
返回相似度 > 0.6 的记忆
```

#### 为什么要搜多次？

一个问题可能漏掉一些角度，多个问题覆盖更全面：

```
问题1 "用户的旅行经历" → 找到 [记忆A, 记忆B]
问题2 "用户的爱好"     → 找到 [记忆B, 记忆C, 记忆D]
问题3 "用户的朋友"     → 找到 [记忆A, 记忆E]
问题4 "用户的计划"     → 找到 [记忆F, 记忆A]
```

合并后：找到 6 条相关记忆（A, B, C, D, E, F）

如果只用一个问题，可能只找到 2 条。

---

### 2.3 功能三：MRR 智能排序（Mean Reciprocal Rank）

#### 是什么？

把多次搜索的结果"投票"排序，被提到次数越多、排名越靠前的记忆得分越高。

#### 怎么计算？

**投票规则：**
- 排第 1 名 → 得 1 分
- 排第 2 名 → 得 0.5 分
- 排第 3 名 → 得 0.33 分
- 排第 4 名 → 得 0.25 分
- 以此类推...

**计算示例：**

```
4 次搜索的结果：

搜索1: [记忆A(第1), 记忆B(第2), 记忆C(第3)]
搜索2: [记忆B(第1), 记忆D(第2), 记忆A(第3)]
搜索3: [记忆A(第1), 记忆E(第2), 记忆F(第3)]
搜索4: [记忆D(第1), 记忆A(第2), 记忆B(第3)]

计算得分：
┌────────┬─────────────────────────────┬────────┐
│  记忆  │          得分计算           │  总分  │
├────────┼─────────────────────────────┼────────┤
│ 记忆A  │ 1 + 0.33 + 1 + 0.5         │  2.83  │ ⭐ 最高
│ 记忆B  │ 0.5 + 1 + 0 + 0.33         │  1.83  │
│ 记忆D  │ 0 + 0.5 + 0 + 1            │  1.50  │
│ 记忆C  │ 0.33 + 0 + 0 + 0           │  0.33  │
│ 记忆E  │ 0 + 0 + 0.5 + 0            │  0.50  │
│ 记忆F  │ 0 + 0 + 0.33 + 0           │  0.33  │
└────────┴─────────────────────────────┴────────┘

最终排序: A > B > D > E > C = F
```

#### 为什么这样排序更好？

- **出现次数多** = 从多个角度都相关 = 更重要
- **排名靠前** = 相似度高 = 更相关
- 结合两者，找出"真正重要"的记忆

---

### 2.4 功能四：记忆分层检索（Tiered Memory Retrieval）

#### 是什么？

把记忆按"使用频率"分成三层：**热层、温层、冷层**。查询时**优先搜索热层**，只有热层不够时才搜温层，冷层不参与日常搜索。

这是参考 Turbopuffer 等高速向量数据库的分层架构设计，目标是将查询延迟从 150ms 降到 50ms。

#### 为什么需要分层？

| 没有分层 | 有分层 |
|---------|--------|
| 每次搜索全部 50 条记忆 | 80% 查询只搜 8 条热记忆 |
| 延迟 150ms | 延迟 50ms |
| 过时信息可能干扰结果 | 只返回最相关的记忆 |

#### 三层定义

```
┌─────────────────────────────────────────────────────────────────┐
│  记忆分层架构                                                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  🔥 HOT TIER（热层）                                            │
│  ─────────────────────────────────────────────────────────────  │
│  条件: 最近 7 天被访问过（last_accessed_at > 7 天前）            │
│  或者: PREF / EFFECTIVE 标签（始终热）                           │
│  查询: 所有搜索首先命中热层                                      │
│  延迟: < 50ms                                                   │
│                                                                 │
│  🌡️ WARM TIER（温层）                                           │
│  ─────────────────────────────────────────────────────────────  │
│  条件: 7-30 天未被访问                                           │
│  查询: 热层结果不够时才搜索温层                                   │
│  延迟: 50-100ms                                                 │
│                                                                 │
│  ❄️ COLD TIER（冷层）                                           │
│  ─────────────────────────────────────────────────────────────  │
│  条件: 30+ 天未被访问，或 compression_status = 'compressed'      │
│  查询: 不参与日常搜索，仅夜间压缩时评估                           │
│  处理: 夜间压缩任务会删除或归档                                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### 什么时候"热层不够"？

用一个真实用户故事来说明：

**用户小明有 50 条记忆**：
- 🔥 热层 8 条（最近 7 天用过 + PREF/EFFECTIVE）
- 🌡️ 温层 15 条（7-30 天没用过）
- ❄️ 冷层 27 条（30+ 天没用过）

**场景 1：热层够用** ✅
```
小明说：「帮我写代码」
    ↓
搜索热层 8 条 → 找到 3 条相关记忆：
  ✅ "小明在写代码时容易分心刷手机" (相似度 0.85)
  ✅ "小明喜欢被温柔鼓励" (相似度 0.72)
  ✅ "倒数 3-2-1 对小明有效" (相似度 0.68)
    ↓
热层返回 3 条高相关记忆 → 够用！不需要查温层
    ↓
AI 回复：「写代码容易分心对吧？要不我们先把手机放远一点，
         然后 3-2-1 开始写第一行代码？」

耗时: ~50ms（只查热层）
```

**场景 2：热层不够（数量不够）** ❌
```
小明说：「我想去健身」
    ↓
搜索热层 8 条 → 没有一条和健身相关！
  ❌ "小明在写代码时容易分心" (相似度 0.15)
  ❌ "小明周末要去迪士尼" (相似度 0.12)
  ❌ ... 都不相关
    ↓
热层返回 0 条 → 不够！需要查温层
    ↓
搜索温层 15 条 → 找到：
  ✅ "小明健身前会找借口拖延" (相似度 0.88)
  ✅ "小明不喜欢被催促" (相似度 0.65)
    ↓
AI 回复：「想去健身啊！我记得你之前健身前会找各种借口，
         今天有什么想拖延的理由吗？说出来我们一起解决~」

耗时: ~100ms（热层 + 温层）
```

**场景 3：热层不够（相关性不够）** ❌
```
小明说：「我想去旅行」
    ↓
搜索热层 8 条 → 找到一些，但相关性低：
  🟡 "小明周末要去迪士尼" (相似度 0.55) ← 勉强相关，但低于阈值
  ❌ "小明最近在学吉他" (相似度 0.20)
    ↓
热层只有 1 条勉强相关，且相似度 < 0.6 → 不够！需要查温层
    ↓
搜索温层 15 条 → 找到：
  ✅ "小明去过优胜美地露营" (相似度 0.82)
  ✅ "小明喜欢自驾游" (相似度 0.78)
    ↓
AI 回复：「又想去旅行啦！上次你去优胜美地露营玩得很开心，
         这次迪士尼也是自驾去吗？」

耗时: ~100ms（热层 + 温层）
```

#### "不够"的判断标准

| 条件 | 阈值 | 说明 |
|------|------|------|
| **数量不够** | 热层返回 < 3 条 | 用户需要至少 3 条记忆来个性化回复 |
| **相关性不够** | 最高相似度 < 0.6 | 低于此阈值的记忆可能不相关 |
| **覆盖面不够** | 只有 1 种标签 | 好的回复需要 PREF + PROC/EMO 等多种记忆 |

#### 分层检索流程

```
用户输入: "我想去健身"
        │
        ▼
┌───────────────────────────────────────────────────────┐
│ Step 1: 搜索热层                                       │
│ WHERE last_accessed_at > NOW() - INTERVAL '7 days'    │
│    OR tag IN ('PREF', 'EFFECTIVE')                    │
│ ORDER BY similarity DESC                              │
│ LIMIT 10                                              │
└───────────────────────────────────────────────────────┘
        │
        ▼
┌───────────────────────────────────────────────────────┐
│ Step 2: 检查是否"够用"                                 │
│                                                       │
│ 够用条件（满足任一）:                                   │
│ ✅ 返回 >= 3 条记忆                                    │
│ ✅ 最高相似度 >= 0.7（非常相关）                        │
│ ✅ 包含 >= 2 种不同标签                                │
└───────────────────────────────────────────────────────┘
        │
        ├─── 够用 ──→ 返回热层结果（耗时 ~50ms）
        │
        └─── 不够 ──→ 继续 Step 3
                      │
                      ▼
┌───────────────────────────────────────────────────────┐
│ Step 3: 搜索温层                                       │
│ WHERE last_accessed_at BETWEEN                        │
│       NOW() - INTERVAL '30 days'                      │
│   AND NOW() - INTERVAL '7 days'                       │
│ ORDER BY similarity DESC                              │
│ LIMIT 10                                              │
└───────────────────────────────────────────────────────┘
        │
        ▼
┌───────────────────────────────────────────────────────┐
│ Step 4: 合并热层 + 温层结果                            │
│ 按 MRR 分数排序，返回 top 10                           │
│ 耗时 ~100ms                                           │
└───────────────────────────────────────────────────────┘
```

#### 访问追踪

每次记忆被检索命中时，更新 `last_accessed_at`，让它保持在热层：

```sql
-- 查询返回结果后，更新被命中的记忆
UPDATE user_memories
SET last_accessed_at = NOW(),
    access_count = COALESCE(access_count, 0) + 1
WHERE id = ANY($matched_memory_ids);
```

#### 数据库字段

```sql
-- 新增字段（如果尚未添加）
ALTER TABLE user_memories
  ADD COLUMN IF NOT EXISTS last_accessed_at TIMESTAMPTZ DEFAULT NOW();

-- 分层查询索引
CREATE INDEX IF NOT EXISTS idx_user_memories_tiered
  ON user_memories(user_id, last_accessed_at DESC)
  WHERE compression_status = 'active';
```

#### 预期效果

| 指标 | 无分层 | 有分层 |
|------|--------|--------|
| 平均查询延迟 | 150ms | **70ms**（80% 热层 + 20% 温层） |
| 搜索数据量 | 全部 active 记忆 | 热层 ~10 条 / 温层 ~20 条 |
| 过时信息干扰 | 有 | 无（冷层不参与搜索） |

#### 与 Turbopuffer 对比

| 特性 | Turbopuffer | 我们的实现 (pgvector) |
|------|-------------|----------------------|
| 热层存储 | 内存 + SSD | PostgreSQL（SSD） |
| 分层自动化 | 自动迁移 | 基于 last_accessed_at 查询 |
| 冷层存储 | S3 对象存储 | 标记为 compressed，不查询 |
| 延迟目标 | < 50ms | < 70ms（热层）/ < 120ms（温层） |

---

### 2.5 功能五：夜间自动清理（Nightly Compression）

#### 是什么？

每天凌晨 3 点，系统自动执行三项清理任务：

1. **删除低价值记忆** - 比如"用户今天喝了咖啡"
2. **合并重复记忆** - 把说的是同一件事的记忆合并
3. **解决矛盾记忆** - 用户前后说法不一致时，保留最新的

#### 任务一：删除低价值记忆

**评分标准：**

| 分数 | 含义 | 例子 | 处理 |
|------|------|------|------|
| 0.0-0.2 | 琐碎、临时性 | "用户今天喝了咖啡" | 删除 |
| 0.3-0.4 | 有一点参考价值 | "用户今天心情不错" | 软删除（隐藏） |
| 0.5-0.7 | 有用的信息 | "用户喜欢早上运动" | 保留 |
| 0.8-1.0 | 核心洞察 | "用户对截止日期焦虑" | 永久保留 |

**判断流程：**

```
系统把记忆发给 AI，问：
"请评估这些记忆的重要性（0-1分）"

AI 返回：
- "用户今天喝了咖啡" → 0.1 分 → 删除
- "用户每天早上需要咖啡才能清醒" → 0.8 分 → 保留
- "用户对未完成的工作感到焦虑" → 0.9 分 → 永久保留
```

#### 任务二：合并重复记忆

**判断流程：**

```
发现这三条记忆很相似：
1. "用户喜欢跑步"
2. "用户经常早上慢跑"
3. "用户说跑步让他放松"

AI 合并成一条：
"用户喜欢跑步，经常早上慢跑，认为跑步能让他放松"
```

#### 任务三：解决矛盾记忆

**判断流程：**

```
发现矛盾：
- 1月的记忆: "用户说最喜欢蓝色"
- 3月的记忆: "用户说最喜欢绿色"

处理方式：
- 旧记忆（1月）→ 标记为"已过时"，保留历史记录
- 新记忆（3月）→ 标记为"当前版本"

AI 以后会说：
✅ "我记得你喜欢绿色"
❌ "我记得你喜欢蓝色"（不会说这个）
```

#### 为什么需要夜间清理？

| 问题 | 不清理会怎样 | 清理后 |
|------|-------------|--------|
| 记忆太多 | 搜索变慢，找到很多无关内容 | 只保留有价值的 |
| 重复记忆 | AI 反复提同一件事 | 精简、不啰嗦 |
| 矛盾记忆 | AI 说法前后不一致 | 始终保持最新信息 |

---

## 3. 系统架构

### 3.1 整体架构图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                          Tolan Memory System Architecture                   │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │                     Edge Functions                                    │  │
│  │                                                                       │  │
│  │  ┌────────────────────┐  ┌─────────────────────┐  ┌────────────────┐ │  │
│  │  │ get-system-        │  │ memory-             │  │ memory-        │ │  │
│  │  │ instruction        │  │ extractor           │  │ compressor     │ │  │
│  │  │ (修改)             │  │ (修改)              │  │ (新增)         │ │  │
│  │  │                    │  │                     │  │                │ │  │
│  │  │ + 问题合成         │  │ + 重要性评分        │  │ + 夜间压缩     │ │  │
│  │  │ + 多次向量搜索     │  │                     │  │ + 矛盾解决     │ │  │
│  │  │ + MRR 融合         │  │                     │  │ + 低价值清理   │ │  │
│  │  └────────────────────┘  └─────────────────────┘  └────────────────┘ │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │                     Database (PostgreSQL + pgvector)                  │  │
│  │                                                                       │  │
│  │  ┌─────────────────────────────────────────────────────────────────┐ │  │
│  │  │ user_memories 表                                                 │ │  │
│  │  │ + importance_score (重要性评分)                                  │ │  │
│  │  │ + version (版本号)                                               │ │  │
│  │  │ + superseded_by (被哪条新记忆替代)                               │ │  │
│  │  │ + compression_status (状态: active/compressed/deleted)           │ │  │
│  │  └─────────────────────────────────────────────────────────────────┘ │  │
│  │                                                                       │  │
│  │  ┌─────────────────────────────────────────────────────────────────┐ │  │
│  │  │ RPC 函数                                                         │ │  │
│  │  │ + multi_query_search_memories() - 多查询向量搜索                 │ │  │
│  │  │ + get_compression_candidates() - 获取待清理的记忆               │ │  │
│  │  │ + mark_memories_compressed() - 标记已清理                       │ │  │
│  │  └─────────────────────────────────────────────────────────────────┘ │  │
│  │                                                                       │  │
│  │  ┌─────────────────────────────────────────────────────────────────┐ │  │
│  │  │ pg_cron 定时任务                                                 │ │  │
│  │  │ + memory_nightly_compression (每天凌晨 3:00 执行)                │ │  │
│  │  └─────────────────────────────────────────────────────────────────┘ │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 3.2 数据流图：记忆检索

```
用户输入: "I'm excited for my trip this weekend"
                │
                ▼
┌───────────────────────────────────────────────────────────────┐
│ Phase 1: Question Synthesis (问题合成)                         │
│ 耗时: ~300ms                                                   │
│                                                                │
│ 输入: "I'm excited for my trip this weekend"                   │
│ 输出:                                                          │
│ [                                                              │
│   "What trips does the user have coming up?",                  │
│   "What outdoor activities does the user enjoy?",              │
│   "Who does the user usually travel with?",                    │
│   "How does the user feel about travel preparation?"           │
│ ]                                                              │
└───────────────────────────────────────────────────────────────┘
                │
                ▼
┌───────────────────────────────────────────────────────────────┐
│ Phase 2: Batch Embedding Generation (批量向量生成)             │
│ 耗时: ~200ms                                                   │
│                                                                │
│ 一次 API 请求，同时处理 4 个问题                                │
│ 返回 4 个 1536 维向量                                          │
└───────────────────────────────────────────────────────────────┘
                │
                ▼
┌───────────────────────────────────────────────────────────────┐
│ Phase 3: Multi-Query Vector Search (多次向量搜索)              │
│ 耗时: ~150ms (并行执行)                                        │
│                                                                │
│ Query 1 → [Memory A (0.92), Memory B (0.85), Memory C (0.78)]  │
│ Query 2 → [Memory B (0.88), Memory D (0.82), Memory A (0.75)]  │
│ Query 3 → [Memory A (0.90), Memory E (0.80), Memory F (0.72)]  │
│ Query 4 → [Memory D (0.86), Memory A (0.81), Memory B (0.74)]  │
└───────────────────────────────────────────────────────────────┘
                │
                ▼
┌───────────────────────────────────────────────────────────────┐
│ Phase 4: MRR Fusion (智能排序)                                 │
│ 耗时: ~5ms                                                     │
│                                                                │
│ 计算得分:                                                      │
│ Memory A: 1/1 + 1/3 + 1/1 + 1/2 = 2.83 ⭐                      │
│ Memory B: 1/2 + 1/1 + 0   + 1/3 = 1.83                         │
│ Memory D: 0   + 1/2 + 0   + 1/1 = 1.50                         │
│ ...                                                            │
│                                                                │
│ 最终排序: [Memory A, Memory B, Memory D, ...]                  │
└───────────────────────────────────────────────────────────────┘
                │
                ▼
┌───────────────────────────────────────────────────────────────┐
│ Phase 5: 注入系统指令                                          │
│                                                                │
│ 取 top-10 记忆 + PREF 记忆 + EFFECTIVE 记忆                    │
│ 格式化后注入到 AI 系统指令                                      │
└───────────────────────────────────────────────────────────────┘
                │
                ▼
        总耗时: ~650-700ms
```

### 3.3 数据流图：夜间压缩

```
                      ┌──────────────────┐
                      │   每晚 3:00 AM   │
                      │   pg_cron 触发   │
                      └────────┬─────────┘
                               │
                               ▼
┌───────────────────────────────────────────────────────────────┐
│ Step 1: 获取所有活跃用户                                       │
│                                                                │
│ SELECT DISTINCT user_id FROM user_memories                     │
│ WHERE compression_status = 'active'                            │
└───────────────────────────────────────────────────────────────┘
                               │
                               ▼
          ┌────────────────────┼────────────────────┐
          │                    │                    │
          ▼                    ▼                    ▼
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│ 用户 A          │  │ 用户 B          │  │ 用户 C          │
│ (顺序处理)      │  │                 │  │                 │
└────────┬────────┘  └────────┬────────┘  └────────┬────────┘
         │                    │                    │
         ▼                    ▼                    ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 2: 评估重要性                                              │
│                                                                 │
│ 获取该用户所有未评分的记忆                                       │
│ 发给 AI 打分 (0-1)                                              │
│                                                                 │
│ "用户今天喝了咖啡" → 0.1                                        │
│ "用户每天早上需要咖啡" → 0.8                                    │
│ "用户对截止日期焦虑" → 0.9                                      │
└─────────────────────────────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 3: 清理低价值记忆                                          │
│                                                                 │
│ importance_score < 0.2 → 永久删除                               │
│ importance_score < 0.4 → 软删除 (compression_status='compressed')│
│ importance_score >= 0.4 → 保留                                  │
└─────────────────────────────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 4: 检测矛盾                                                │
│                                                                 │
│ 按 tag 分组记忆                                                 │
│ 让 AI 检查每组内是否有矛盾                                      │
│                                                                 │
│ 发现矛盾:                                                       │
│ - 记忆1 (1月): "喜欢蓝色"                                       │
│ - 记忆2 (3月): "喜欢绿色"                                       │
│                                                                 │
│ 处理:                                                           │
│ - 记忆1.superseded_by = 记忆2.id                                │
│ - 记忆1.compression_status = 'compressed'                       │
│ - 记忆2.version = 2                                             │
└─────────────────────────────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 5: 合并重复                                                │
│                                                                 │
│ 使用向量相似度找出重复记忆 (相似度 > 0.85)                       │
│ 让 AI 合并成一条                                                │
│                                                                 │
│ 合并前:                                                         │
│ - "用户喜欢跑步"                                                │
│ - "用户经常早上慢跑"                                            │
│ - "用户说跑步让他放松"                                          │
│                                                                 │
│ 合并后:                                                         │
│ - "用户喜欢跑步，经常早上慢跑，认为跑步能让他放松"               │
│ - merged_from = [原记忆1.id, 原记忆2.id, 原记忆3.id]            │
└─────────────────────────────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 6: 生成报告                                                │
│                                                                 │
│ {                                                               │
│   "usersProcessed": 150,                                        │
│   "totalEvaluated": 2340,                                       │
│   "totalDeleted": 234,                                          │
│   "totalCompressed": 456,                                       │
│   "contradictionsResolved": 23,                                 │
│   "mergedGroups": 89                                            │
│ }                                                               │
└─────────────────────────────────────────────────────────────────┘
```

---

## 4. 数据库设计

### 4.1 表结构变更

**修改表: user_memories**

```sql
-- 新增字段
ALTER TABLE user_memories ADD COLUMN IF NOT EXISTS importance_score FLOAT DEFAULT 0.5;
ALTER TABLE user_memories ADD COLUMN IF NOT EXISTS version INTEGER DEFAULT 1;
ALTER TABLE user_memories ADD COLUMN IF NOT EXISTS superseded_by UUID REFERENCES user_memories(id);
ALTER TABLE user_memories ADD COLUMN IF NOT EXISTS compression_status TEXT DEFAULT 'active';

-- 约束
ALTER TABLE user_memories ADD CONSTRAINT compression_status_check
  CHECK (compression_status IN ('active', 'compressed', 'deleted'));
```

**字段说明:**

| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `importance_score` | FLOAT | 0.5 | 重要性评分 (0-1)，由 AI 评估 |
| `version` | INTEGER | 1 | 版本号，矛盾解决时递增 |
| `superseded_by` | UUID | NULL | 被哪条新记忆替代（矛盾解决时设置） |
| `compression_status` | TEXT | 'active' | 状态：active=活跃，compressed=已压缩，deleted=已删除 |

### 4.2 新增索引

```sql
-- 按重要性排序的索引（只索引活跃记忆）
CREATE INDEX idx_user_memories_importance
  ON user_memories(user_id, importance_score DESC)
  WHERE compression_status = 'active';

-- 压缩任务专用索引
CREATE INDEX idx_user_memories_compression
  ON user_memories(compression_status, updated_at)
  WHERE compression_status = 'active';
```

### 4.3 新增 RPC 函数

#### 函数 1: multi_query_search_memories

**功能:** 支持多个 embedding 同时搜索，返回带排名的结果

**参数:**

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `p_user_id` | UUID | - | 用户 ID |
| `p_embeddings` | TEXT[] | - | JSON 格式的 embedding 数组 |
| `p_threshold` | FLOAT | 0.6 | 相似度阈值 |
| `p_limit_per_query` | INTEGER | 5 | 每个查询返回的最大结果数 |

**返回:**

| 字段 | 类型 | 说明 |
|------|------|------|
| `query_index` | INTEGER | 第几个查询 (1-based) |
| `memory_id` | UUID | 记忆 ID |
| `content` | TEXT | 记忆内容 |
| `tag` | TEXT | 标签 |
| `confidence` | FLOAT | 置信度 |
| `importance_score` | FLOAT | 重要性评分 |
| `similarity` | FLOAT | 相似度 |
| `rank` | INTEGER | 在该查询中的排名 |

#### 函数 2: get_compression_candidates

**功能:** 获取待清理的记忆列表

**参数:**

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `p_user_id` | UUID | - | 用户 ID |
| `p_min_age_days` | INTEGER | 7 | 最小天数（避免清理太新的记忆） |
| `p_low_importance_threshold` | FLOAT | 0.3 | 低重要性阈值 |
| `p_limit` | INTEGER | 100 | 最大返回数量 |

**返回:**

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | UUID | 记忆 ID |
| `content` | TEXT | 记忆内容 |
| `tag` | TEXT | 标签 |
| `importance_score` | FLOAT | 重要性评分 |
| `access_count` | INTEGER | 访问次数 |
| `created_at` | TIMESTAMPTZ | 创建时间 |
| `reason` | TEXT | 被选中的原因 |

#### 函数 3: mark_memories_compressed

**功能:** 批量标记记忆为已压缩或删除

**参数:**

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `p_memory_ids` | UUID[] | - | 要处理的记忆 ID 数组 |
| `p_action` | TEXT | 'compress' | 操作类型：compress=软删除，delete=永久删除 |

**返回:** INTEGER (受影响的行数)

---

## 5. Edge Function 实现

### 5.1 修改: get-system-instruction/index.ts

#### 新增函数清单

| 函数名 | 行号(预估) | 功能 |
|--------|-----------|------|
| `synthesizeQuestions()` | ~65-120 | 问题合成 |
| `generateEmbeddings()` | ~125-170 | 批量生成 embedding |
| `mergeWithMRR()` | ~175-210 | MRR 融合排序 |
| `multiQueryRAG()` | ~215-280 | 整合多查询+融合 |

#### synthesizeQuestions 函数

```typescript
/**
 * 问题合成 - 从用户输入生成检索问题
 *
 * @param taskDescription - 任务描述或用户当前发言
 * @param recentContext - 可选的最近上下文
 * @returns 3-5 个检索问题
 */
async function synthesizeQuestions(
  taskDescription: string,
  recentContext?: string
): Promise<string[]> {
  const AZURE_ENDPOINT = Deno.env.get('AZURE_AI_ENDPOINT');
  const AZURE_API_KEY = Deno.env.get('AZURE_AI_API_KEY');
  const MODEL_NAME = Deno.env.get('MEMORY_EXTRACTOR_MODEL') || 'gpt-5.1-chat';

  // 如果没有配置 API，回退到原始查询
  if (!AZURE_API_KEY) {
    console.warn('Azure API Key not set, skipping question synthesis');
    return [taskDescription];
  }

  const prompt = `You are a memory retrieval assistant. Given the user's current task or statement, generate 3-5 search questions that would help retrieve relevant memories about the user.

Current task/statement: "${taskDescription}"
${recentContext ? `Recent context: "${recentContext}"` : ''}

Generate questions that would help understand:
1. What the user has done/said about similar topics before
2. Their preferences and patterns related to this
3. Their emotional responses to similar situations
4. Any struggles or successes with related tasks

Return a JSON array of strings. Each string should be a complete question.
Example output: ["What exercises does the user prefer?", "How does the user feel about working out?"]

IMPORTANT:
- Questions should be about the USER, not general questions
- Keep questions specific and actionable
- Use simple, searchable language`;

  try {
    const response = await fetch(`${AZURE_ENDPOINT}/openai/v1/chat/completions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${AZURE_API_KEY}`,
      },
      body: JSON.stringify({
        model: MODEL_NAME,
        messages: [{ role: 'user', content: prompt }],
        max_completion_tokens: 300,
        temperature: 0.3, // 低温度确保输出稳定
      }),
    });

    if (!response.ok) {
      console.warn('Question synthesis failed:', response.status);
      return [taskDescription];
    }

    const result = await response.json();
    const content = result.choices?.[0]?.message?.content;

    try {
      const questions = JSON.parse(content);
      if (Array.isArray(questions) && questions.length > 0) {
        console.log(`🔍 Generated ${questions.length} retrieval questions`);
        return questions.slice(0, 5); // 最多 5 个问题
      }
    } catch {
      console.warn('Failed to parse questions, using fallback');
    }

    return [taskDescription];
  } catch (error) {
    console.error('Question synthesis error:', error);
    return [taskDescription];
  }
}
```

#### generateEmbeddings 函数

```typescript
/**
 * 批量生成 embeddings
 *
 * @param texts - 要转换的文本数组
 * @returns embedding 数组，每个是 1536 维向量
 */
async function generateEmbeddings(texts: string[]): Promise<number[][]> {
  const EMBEDDING_ENDPOINT = Deno.env.get('AZURE_EMBEDDING_ENDPOINT')
    || Deno.env.get('AZURE_AI_ENDPOINT');
  const EMBEDDING_API_KEY = Deno.env.get('AZURE_EMBEDDING_API_KEY')
    || Deno.env.get('AZURE_AI_API_KEY');
  const EMBEDDING_MODEL = Deno.env.get('MEMORY_EMBEDDING_MODEL')
    || 'text-embedding-3-small';

  if (!EMBEDDING_API_KEY) {
    throw new Error('Embedding API key not configured');
  }

  const response = await fetch(`${EMBEDDING_ENDPOINT}/embeddings`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${EMBEDDING_API_KEY}`,
    },
    body: JSON.stringify({
      model: EMBEDDING_MODEL,
      input: texts,  // 批量输入
      dimensions: 1536,
    }),
  });

  if (!response.ok) {
    throw new Error(`Embedding request failed: ${response.status}`);
  }

  const result = await response.json();
  return result.data.map((item: { embedding: number[] }) => item.embedding);
}
```

#### mergeWithMRR 函数

```typescript
interface MemorySearchResult {
  query_index: number;
  memory_id: string;
  content: string;
  tag: string;
  confidence: number;
  importance_score: number;
  similarity: number;
  rank: number;
}

/**
 * Mean Reciprocal Rank 融合
 *
 * @param resultSets - 多次搜索的结果，按 query_index 分组
 * @returns 按 MRR 分数排序的唯一记忆列表
 */
function mergeWithMRR(results: MemorySearchResult[]): MemorySearchResult[] {
  const scores = new Map<string, { score: number; memory: MemorySearchResult }>();

  // 按 query_index 分组
  const byQuery = new Map<number, MemorySearchResult[]>();
  for (const r of results) {
    if (!byQuery.has(r.query_index)) {
      byQuery.set(r.query_index, []);
    }
    byQuery.get(r.query_index)!.push(r);
  }

  // 计算 MRR 分数
  for (const [queryIndex, queryResults] of byQuery) {
    for (const memory of queryResults) {
      // Reciprocal Rank = 1 / rank
      const rr = 1 / memory.rank;
      const existing = scores.get(memory.memory_id);

      if (existing) {
        existing.score += rr;
      } else {
        scores.set(memory.memory_id, { score: rr, memory });
      }
    }
  }

  // 按 MRR 分数排序
  return [...scores.values()]
    .sort((a, b) => b.score - a.score)
    .map(item => item.memory);
}
```

#### multiQueryRAG 函数

```typescript
/**
 * Multi-Query RAG 检索
 *
 * @param supabase - Supabase 客户端
 * @param userId - 用户 ID
 * @param questions - 检索问题列表
 * @param limit - 最终返回的记忆数量
 * @returns 格式化的记忆字符串数组
 */
async function multiQueryRAG(
  supabase: ReturnType<typeof createClient>,
  userId: string,
  questions: string[],
  limit: number = 10
): Promise<string[]> {
  try {
    console.log(`🔍 Multi-Query RAG: ${questions.length} questions`);

    // 1. 并行生成所有问题的 embeddings
    const startEmbed = Date.now();
    const embeddings = await generateEmbeddings(questions);
    console.log(`⏱️ Embedding generation: ${Date.now() - startEmbed}ms`);

    // 2. 调用 multi_query_search_memories RPC
    const embeddingsJson = embeddings.map(e => JSON.stringify(e));

    const startSearch = Date.now();
    const { data, error } = await supabase.rpc('multi_query_search_memories', {
      p_user_id: userId,
      p_embeddings: embeddingsJson,
      p_threshold: 0.6, // 比单查询更宽松的阈值
      p_limit_per_query: 5,
    });
    console.log(`⏱️ Vector search: ${Date.now() - startSearch}ms`);

    if (error) {
      console.error('Multi-query search failed:', error);
      return [];
    }

    // 3. MRR 融合
    const startMRR = Date.now();
    const fusedResults = mergeWithMRR(data || []);
    console.log(`⏱️ MRR fusion: ${Date.now() - startMRR}ms`);
    console.log(`📊 MRR fusion: ${fusedResults.length} unique memories`);

    // 4. 格式化返回
    const tagContext: Record<string, string> = {
      'PREF': '(AI 交互偏好)',
      'PROC': '(拖延模式)',
      'SOMA': '(身心反应)',
      'EMO': '(情绪模式)',
      'SAB': '(自我妨碍)',
      'EFFECTIVE': '(有效激励方式)',
    };

    return fusedResults.slice(0, limit).map(m => {
      const context = tagContext[m.tag] || '';
      return `${m.content} ${context}`.trim();
    });
  } catch (error) {
    console.error('Multi-Query RAG error:', error);
    return [];
  }
}
```

#### 重构 getUserMemories 函数

```typescript
/**
 * 获取用户记忆 - Tolan 升级版
 *
 * 流程:
 * 1. 始终加载 PREF + EFFECTIVE 记忆
 * 2. Question Synthesis: 生成检索问题
 * 3. Multi-Query RAG: 并行向量搜索
 * 4. MRR Fusion: 融合排序结果
 */
async function getUserMemories(
  supabase: ReturnType<typeof createClient>,
  userId: string,
  taskDescription: string,
  limit = 10
): Promise<string[]> {
  const ENABLE_TOLAN = Deno.env.get('ENABLE_TOLAN_MEMORY') === 'true';

  // 如果未启用 Tolan，使用原有逻辑
  if (!ENABLE_TOLAN) {
    return await getUserMemoriesLegacy(supabase, userId, taskDescription, limit);
  }

  try {
    const memories: string[] = [];
    const startTotal = Date.now();

    // ==========================================
    // Phase 0: 始终加载的记忆 (PREF + EFFECTIVE)
    // ==========================================

    // PREF 全量加载（不限条数）
    const { data: prefMemories } = await supabase
      .from('user_memories')
      .select('content, tag')
      .eq('user_id', userId)
      .eq('tag', 'PREF')
      .eq('compression_status', 'active')
      .gte('confidence', 0.5)
      .order('importance_score', { ascending: false });

    if (prefMemories) {
      memories.push(...prefMemories.map(m => `${m.content} (AI 交互偏好)`));
      console.log(`🧠 PREF memories: ${prefMemories.length}`);
    }

    // EFFECTIVE 最多 5 条
    const { data: effectiveMemories } = await supabase
      .from('user_memories')
      .select('content, tag')
      .eq('user_id', userId)
      .eq('tag', 'EFFECTIVE')
      .eq('compression_status', 'active')
      .gte('confidence', 0.5)
      .order('importance_score', { ascending: false })
      .limit(5);

    if (effectiveMemories) {
      memories.push(...effectiveMemories.map(m => `${m.content} (有效激励方式)`));
      console.log(`🧠 EFFECTIVE memories: ${effectiveMemories.length}`);
    }

    // ==========================================
    // Phase 1: Question Synthesis
    // ==========================================
    const questions = await synthesizeQuestions(taskDescription);
    console.log(`🔍 Synthesized questions: ${questions.length}`);

    // ==========================================
    // Phase 2-4: Multi-Query RAG + MRR Fusion
    // ==========================================
    const ragLimit = Math.max(1, limit - memories.length);
    const ragMemories = await multiQueryRAG(supabase, userId, questions, ragLimit);

    // 去重（避免和 PREF/EFFECTIVE 重复）
    const existingContents = new Set(memories.map(m => m.split(' (')[0]));
    const uniqueRagMemories = ragMemories.filter(m =>
      !existingContents.has(m.split(' (')[0])
    );

    memories.push(...uniqueRagMemories);

    console.log(`🧠 Final memories: ${memories.length}`);
    console.log(`⏱️ Total retrieval time: ${Date.now() - startTotal}ms`);

    return memories.slice(0, limit);
  } catch (error) {
    console.warn('Tolan retrieval failed, falling back to legacy:', error);
    return await getUserMemoriesLegacy(supabase, userId, taskDescription, limit);
  }
}

// 保留原有函数作为降级方案
async function getUserMemoriesLegacy(
  supabase: ReturnType<typeof createClient>,
  userId: string,
  taskDescription: string,
  limit = 10
): Promise<string[]> {
  // ... 原有的 textSearch 逻辑
}
```

### 5.2 新建: memory-compressor/index.ts

完整代码见 [附录 10.2](#102-memory-compressorindexts-完整代码)

#### 核心函数清单

| 函数名 | 功能 |
|--------|------|
| `evaluateImportance()` | 使用 AI 评估记忆重要性 (0-1) |
| `resolveContradictions()` | 检测并解决矛盾记忆 |
| `compressLowValueMemories()` | 清理低价值记忆 |
| `mergeRedundantMemories()` | 合并重复记忆 |

#### API 接口

**请求格式:**

```json
// 压缩所有用户（由 cron 调用）
{
  "action": "compress_all"
}

// 压缩单个用户
{
  "action": "compress_user",
  "userId": "xxx-xxx-xxx"
}

// 只评估不执行（调试用）
{
  "action": "dry_run",
  "userId": "xxx-xxx-xxx"
}
```

**响应格式:**

```json
{
  "success": true,
  "usersProcessed": 150,
  "totalEvaluated": 2340,
  "totalDeleted": 234,
  "totalCompressed": 456,
  "contradictionsResolved": 23,
  "mergedGroups": 89,
  "durationMs": 45000
}
```

### 5.3 修改: memory-extractor/index.ts

#### 添加重要性评分

在 `saveOrMergeMemories` 函数中，为新记忆计算初始重要性评分：

```typescript
// 在插入新记忆时添加 importance_score
const { data: newMemory, error: insertError } = await supabase
  .from('user_memories')
  .insert({
    user_id: userId,
    content: memory.content,
    tag: memory.tag,
    confidence: memory.confidence,
    embedding: embedding,
    task_name: taskDescription || null,
    // 新增：根据置信度和标签计算初始重要性
    importance_score: calculateInitialImportance(memory),
  })
  .select()
  .single();

// 新增函数
function calculateInitialImportance(memory: ExtractedMemory): number {
  let score = memory.confidence; // 基础分 = 置信度

  // PREF 和 EFFECTIVE 类型加分
  if (memory.tag === 'PREF' || memory.tag === 'EFFECTIVE') {
    score = Math.min(1, score + 0.2);
  }

  return score;
}
```

---

## 6. 性能优化

### 6.1 延迟预算

| 步骤 | 当前延迟 | 优化后目标 | 优化方法 |
|------|---------|-----------|---------|
| Question Synthesis | 新增 | ~300ms | 低温度 + 短 max_tokens |
| Embedding Generation | N×200ms | ~200ms | 批量 API (单次请求多个 input) |
| Multi-Query Search | N×100ms | ~150ms | 并行 RPC + 数据库连接池 |
| MRR Fusion | - | ~5ms | 内存中计算，无 IO |
| **总计** | - | **~650ms** | - |

### 6.2 缓存策略

```typescript
// 简单的内存缓存
const memoryCache = new Map<string, {
  data: string[];
  expires: number
}>();

const CACHE_TTL_MS = 5 * 60 * 1000; // 5 分钟

async function getUserMemoriesWithCache(
  supabase: ReturnType<typeof createClient>,
  userId: string,
  taskDescription: string
): Promise<string[]> {
  const cacheKey = `${userId}:${taskDescription}`;
  const cached = memoryCache.get(cacheKey);

  if (cached && cached.expires > Date.now()) {
    console.log('🚀 Memory cache hit');
    return cached.data;
  }

  const memories = await getUserMemories(supabase, userId, taskDescription);

  memoryCache.set(cacheKey, {
    data: memories,
    expires: Date.now() + CACHE_TTL_MS,
  });

  return memories;
}
```

### 6.3 降级策略

```typescript
const ENABLE_TOLAN = Deno.env.get('ENABLE_TOLAN_MEMORY') === 'true';

async function getUserMemoriesWithFallback(
  supabase: ReturnType<typeof createClient>,
  userId: string,
  taskDescription: string,
  limit: number
): Promise<string[]> {
  if (!ENABLE_TOLAN) {
    return await getUserMemoriesLegacy(supabase, userId, taskDescription, limit);
  }

  try {
    return await getUserMemories(supabase, userId, taskDescription, limit);
  } catch (error) {
    console.warn('Tolan retrieval failed, falling back to simple search:', error);
    return await getUserMemoriesLegacy(supabase, userId, taskDescription, limit);
  }
}
```

---

## 7. 部署配置

### 7.1 环境变量

```bash
# 功能开关
ENABLE_TOLAN_MEMORY=true

# Azure OpenAI (已有)
AZURE_AI_ENDPOINT=https://your-endpoint.openai.azure.com
AZURE_AI_API_KEY=your-api-key
MEMORY_EXTRACTOR_MODEL=gpt-5.1-chat

# Embedding (已有)
AZURE_EMBEDDING_ENDPOINT=https://your-embedding-endpoint.openai.azure.com
AZURE_EMBEDDING_API_KEY=your-embedding-api-key
MEMORY_EMBEDDING_MODEL=text-embedding-3-large

# 夜间压缩配置
COMPRESSION_LOW_IMPORTANCE_THRESHOLD=0.3
COMPRESSION_MIN_AGE_DAYS=7
```

### 7.2 数据库迁移命令

```bash
# 应用迁移
supabase db push

# 或手动执行
psql -h <host> -U postgres -d postgres -f supabase/migrations/20260127100000_tolan_memory_system.sql
psql -h <host> -U postgres -d postgres -f supabase/migrations/20260127110000_memory_compression_cron.sql
```

### 7.3 部署 Edge Functions

```bash
# 部署修改的函数
supabase functions deploy get-system-instruction
supabase functions deploy memory-extractor

# 部署新函数
supabase functions deploy memory-compressor
```

### 7.4 验证 Cron 任务

```sql
-- 查看已注册的 cron 任务
SELECT * FROM cron.job;

-- 手动触发一次（测试用）
SELECT cron.schedule('test_compression', 'now',
  $$ SELECT net.http_post(...) $$
);

-- 取消测试任务
SELECT cron.unschedule('test_compression');
```

---

## 8. 验证计划

### 8.1 Phase 1 验收标准

| 验收项 | 标准 | 验证方法 |
|--------|------|---------|
| 问题合成 | 能生成 3-5 个有意义的问题 | 查看日志输出 |
| 多查询搜索 | 每个问题返回 ≤5 条结果 | 查看 RPC 返回 |
| MRR 融合 | 结果按分数正确排序 | 单元测试 |
| 端到端延迟 | < 1000ms | 日志计时 |
| 回归测试 | 原有功能不受影响 | 手动测试 |

### 8.2 Phase 2 验收标准

| 验收项 | 标准 | 验证方法 |
|--------|------|---------|
| Cron 任务 | 每天 3:00 UTC 自动执行 | 查看 cron.job_run_details |
| 重要性评分 | 正确识别低价值记忆 | 抽样检查 |
| 矛盾解决 | 保留最新版本 | 查看 superseded_by 字段 |
| 合并重复 | merged_from 记录正确 | 查看 metadata |

### 8.3 测试用例

```typescript
// 测试 Question Synthesis
const questions = await synthesizeQuestions("我周末要去旅行");
assert(questions.length >= 3 && questions.length <= 5);
assert(questions.every(q => q.includes("用户") || q.includes("user")));

// 测试 MRR Fusion
const mockResults = [
  { query_index: 1, memory_id: 'A', rank: 1, ... },
  { query_index: 1, memory_id: 'B', rank: 2, ... },
  { query_index: 2, memory_id: 'B', rank: 1, ... },
  { query_index: 2, memory_id: 'A', rank: 3, ... },
];
const fused = mergeWithMRR(mockResults);
assert(fused[0].memory_id === 'B'); // B 得分最高: 0.5 + 1 = 1.5
assert(fused[1].memory_id === 'A'); // A 得分: 1 + 0.33 = 1.33
```

---

## 9. 回滚策略

### 9.1 即时回滚（功能开关）

```bash
# 禁用 Tolan 功能，立即回退到原有逻辑
ENABLE_TOLAN_MEMORY=false
```

### 9.2 暂停 Cron 任务

```sql
-- 暂停夜间压缩
SELECT cron.unschedule('memory_nightly_compression');

-- 恢复
SELECT cron.schedule('memory_nightly_compression', '0 3 * * *', ...);
```

### 9.3 数据库回滚

新增的字段不影响原有代码，无需回滚。如果确实需要：

```sql
-- 恢复被压缩的记忆
UPDATE user_memories
SET compression_status = 'active'
WHERE compression_status = 'compressed';

-- 删除新字段（谨慎！）
ALTER TABLE user_memories DROP COLUMN importance_score;
ALTER TABLE user_memories DROP COLUMN version;
ALTER TABLE user_memories DROP COLUMN superseded_by;
ALTER TABLE user_memories DROP COLUMN compression_status;
```

---

## 10. 附录：完整代码

### 10.1 数据库迁移完整 SQL

**文件: `supabase/migrations/20260127100000_tolan_memory_system.sql`**

```sql
-- =====================================================
-- Tolan Memory System Upgrade
-- 功能: 问题合成、多查询 RAG、MRR 融合、夜间压缩
-- 创建日期: 2026-01-27
-- =====================================================

-- 1. 扩展 user_memories 表
-- =====================================================

-- 重要性评分 (0-1)
ALTER TABLE user_memories
  ADD COLUMN IF NOT EXISTS importance_score FLOAT DEFAULT 0.5;

-- 版本号 (矛盾解决时递增)
ALTER TABLE user_memories
  ADD COLUMN IF NOT EXISTS version INTEGER DEFAULT 1;

-- 被替代的新记忆 ID
ALTER TABLE user_memories
  ADD COLUMN IF NOT EXISTS superseded_by UUID REFERENCES user_memories(id);

-- 压缩状态
ALTER TABLE user_memories
  ADD COLUMN IF NOT EXISTS compression_status TEXT DEFAULT 'active';

-- 添加约束
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'user_memories_compression_status_check'
  ) THEN
    ALTER TABLE user_memories
      ADD CONSTRAINT user_memories_compression_status_check
      CHECK (compression_status IN ('active', 'compressed', 'deleted'));
  END IF;
END $$;

-- 添加字段注释
COMMENT ON COLUMN user_memories.importance_score IS '记忆重要性评分 0-1，由 AI 评估，用于压缩决策';
COMMENT ON COLUMN user_memories.version IS '记忆版本号，矛盾解决时递增，用于保留历史';
COMMENT ON COLUMN user_memories.superseded_by IS '被哪条新记忆替代（矛盾解决时设置）';
COMMENT ON COLUMN user_memories.compression_status IS '压缩状态: active=活跃, compressed=已压缩, deleted=已删除';

-- 2. 创建索引
-- =====================================================

-- 按重要性排序的索引（只索引活跃记忆）
CREATE INDEX IF NOT EXISTS idx_user_memories_importance
  ON user_memories(user_id, importance_score DESC)
  WHERE compression_status = 'active';

-- 压缩任务专用索引
CREATE INDEX IF NOT EXISTS idx_user_memories_compression
  ON user_memories(compression_status, updated_at)
  WHERE compression_status = 'active';

-- 3. 多查询向量搜索函数
-- =====================================================

CREATE OR REPLACE FUNCTION multi_query_search_memories(
  p_user_id UUID,
  p_embeddings TEXT[],  -- JSON 格式的 embedding 数组
  p_threshold FLOAT DEFAULT 0.6,
  p_limit_per_query INTEGER DEFAULT 5
)
RETURNS TABLE (
  query_index INTEGER,
  memory_id UUID,
  content TEXT,
  tag TEXT,
  confidence FLOAT,
  importance_score FLOAT,
  similarity FLOAT,
  rank INTEGER
)
LANGUAGE plpgsql
AS $$
DECLARE
  v_embedding_json TEXT;
  v_embedding vector(1536);
  v_query_index INTEGER := 0;
BEGIN
  -- 遍历每个 embedding
  FOREACH v_embedding_json IN ARRAY p_embeddings
  LOOP
    v_query_index := v_query_index + 1;

    -- 将 JSON 字符串转换为 vector
    v_embedding := v_embedding_json::vector(1536);

    -- 执行向量搜索并返回结果
    RETURN QUERY
    WITH ranked AS (
      SELECT
        m.id AS memory_id,
        m.content,
        m.tag,
        m.confidence,
        COALESCE(m.importance_score, 0.5) AS importance_score,
        1 - (m.embedding <=> v_embedding) AS similarity,
        ROW_NUMBER() OVER (
          ORDER BY (1 - (m.embedding <=> v_embedding)) DESC
        )::INTEGER AS rank
      FROM user_memories m
      WHERE m.user_id = p_user_id
        AND m.embedding IS NOT NULL
        AND (m.compression_status IS NULL OR m.compression_status = 'active')
        AND (1 - (m.embedding <=> v_embedding)) >= p_threshold
      LIMIT p_limit_per_query
    )
    SELECT v_query_index, ranked.* FROM ranked;
  END LOOP;
END;
$$;

COMMENT ON FUNCTION multi_query_search_memories IS
  '多查询向量搜索，支持批量 embedding 并行搜索，返回带排名的结果用于 MRR 融合';

-- 4. 获取压缩候选记忆函数
-- =====================================================

CREATE OR REPLACE FUNCTION get_compression_candidates(
  p_user_id UUID,
  p_min_age_days INTEGER DEFAULT 7,
  p_low_importance_threshold FLOAT DEFAULT 0.3,
  p_limit INTEGER DEFAULT 100
)
RETURNS TABLE (
  id UUID,
  content TEXT,
  tag TEXT,
  importance_score FLOAT,
  access_count INTEGER,
  created_at TIMESTAMPTZ,
  reason TEXT
)
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY
  SELECT
    m.id,
    m.content,
    m.tag,
    COALESCE(m.importance_score, 0.5) AS importance_score,
    COALESCE(m.access_count, 0) AS access_count,
    m.created_at,
    CASE
      WHEN COALESCE(m.importance_score, 0.5) < p_low_importance_threshold
        THEN 'low_importance'
      WHEN COALESCE(m.access_count, 0) = 0
        AND m.created_at < NOW() - (p_min_age_days || ' days')::INTERVAL
        THEN 'never_accessed'
      ELSE 'other'
    END AS reason
  FROM user_memories m
  WHERE m.user_id = p_user_id
    AND (m.compression_status IS NULL OR m.compression_status = 'active')
    AND (
      -- 低重要性记忆
      COALESCE(m.importance_score, 0.5) < p_low_importance_threshold
      -- 或：从未被访问且超过 N 天
      OR (
        COALESCE(m.access_count, 0) = 0
        AND m.created_at < NOW() - (p_min_age_days || ' days')::INTERVAL
      )
    )
  ORDER BY COALESCE(m.importance_score, 0.5) ASC, COALESCE(m.access_count, 0) ASC
  LIMIT p_limit;
END;
$$;

COMMENT ON FUNCTION get_compression_candidates IS
  '获取待压缩的低价值记忆候选，基于重要性评分和访问频率';

-- 5. 标记记忆为已压缩函数
-- =====================================================

CREATE OR REPLACE FUNCTION mark_memories_compressed(
  p_memory_ids UUID[],
  p_action TEXT DEFAULT 'compress'  -- 'compress' 或 'delete'
)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
  v_count INTEGER;
BEGIN
  IF p_action = 'delete' THEN
    -- 永久删除
    DELETE FROM user_memories WHERE id = ANY(p_memory_ids);
  ELSE
    -- 软删除（标记为已压缩）
    UPDATE user_memories
    SET
      compression_status = 'compressed',
      updated_at = NOW()
    WHERE id = ANY(p_memory_ids);
  END IF;

  GET DIAGNOSTICS v_count = ROW_COUNT;
  RETURN v_count;
END;
$$;

COMMENT ON FUNCTION mark_memories_compressed IS
  '批量标记记忆为已压缩或永久删除';

-- 6. 获取用户所有活跃记忆（按标签分组）
-- =====================================================

CREATE OR REPLACE FUNCTION get_user_memories_by_tag(
  p_user_id UUID,
  p_tag TEXT DEFAULT NULL
)
RETURNS TABLE (
  id UUID,
  content TEXT,
  tag TEXT,
  confidence FLOAT,
  importance_score FLOAT,
  created_at TIMESTAMPTZ
)
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY
  SELECT
    m.id,
    m.content,
    m.tag,
    m.confidence,
    COALESCE(m.importance_score, 0.5) AS importance_score,
    m.created_at
  FROM user_memories m
  WHERE m.user_id = p_user_id
    AND (m.compression_status IS NULL OR m.compression_status = 'active')
    AND (p_tag IS NULL OR m.tag = p_tag)
  ORDER BY m.created_at DESC;
END;
$$;

COMMENT ON FUNCTION get_user_memories_by_tag IS
  '获取用户所有活跃记忆，可按标签筛选';
```

**文件: `supabase/migrations/20260127110000_memory_compression_cron.sql`**

```sql
-- =====================================================
-- Memory Compression Cron Job
-- 每天凌晨 3:00 UTC 执行夜间压缩
-- =====================================================

-- 启用 pg_cron 扩展（如果尚未启用）
CREATE EXTENSION IF NOT EXISTS pg_cron;

-- 启用 pg_net 扩展（用于 HTTP 调用）
CREATE EXTENSION IF NOT EXISTS pg_net;

-- 创建夜间压缩任务
-- 注意：需要先设置 app.settings.supabase_url 和 app.settings.service_role_key
SELECT cron.schedule(
  'memory_nightly_compression',  -- 任务名称
  '0 3 * * *',                   -- Cron 表达式：每天 3:00 UTC
  $$
  SELECT net.http_post(
    url := current_setting('app.settings.supabase_url') || '/functions/v1/memory-compressor',
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer ' || current_setting('app.settings.service_role_key')
    ),
    body := '{"action": "compress_all"}'::jsonb
  );
  $$
);

-- 查看已注册的任务
-- SELECT * FROM cron.job;

-- 查看任务执行历史
-- SELECT * FROM cron.job_run_details ORDER BY start_time DESC LIMIT 10;

-- 如需取消任务
-- SELECT cron.unschedule('memory_nightly_compression');
```

### 10.2 memory-compressor/index.ts 完整代码

```typescript
import { serve } from "https://deno.land/std@0.168.0/http/server.ts"
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
}

// 配置
const AZURE_ENDPOINT = Deno.env.get('AZURE_AI_ENDPOINT');
const AZURE_API_KEY = Deno.env.get('AZURE_AI_API_KEY');
const MODEL_NAME = Deno.env.get('MEMORY_EXTRACTOR_MODEL') || 'gpt-5.1-chat';
const LOW_IMPORTANCE_THRESHOLD = parseFloat(
  Deno.env.get('COMPRESSION_LOW_IMPORTANCE_THRESHOLD') || '0.3'
);
const MIN_AGE_DAYS = parseInt(
  Deno.env.get('COMPRESSION_MIN_AGE_DAYS') || '7'
);

interface Memory {
  id: string;
  content: string;
  tag: string;
  confidence?: number;
  importance_score?: number;
  access_count?: number;
  created_at?: string;
}

interface CompressionResult {
  evaluated: number;
  deleted: number;
  compressed: number;
  contradictionsResolved: number;
  mergedGroups: number;
}

/**
 * 使用 AI 评估记忆的重要性
 */
async function evaluateImportance(
  memories: Memory[]
): Promise<Map<string, number>> {
  if (!AZURE_API_KEY || memories.length === 0) {
    return new Map(memories.map(m => [m.id, 0.5]));
  }

  const prompt = `You are a memory importance evaluator. Rate each memory's importance for understanding a user's behavior patterns and preferences.

Memories to evaluate:
${memories.map((m, i) => `${i + 1}. [${m.tag}] ${m.content}`).join('\n')}

For each memory, rate its importance from 0.0 to 1.0:
- 0.0-0.2: Trivial, ephemeral (e.g., "user drank coffee today", "user said hi")
- 0.3-0.5: Somewhat useful context (e.g., "user was tired today")
- 0.6-0.8: Important pattern or preference (e.g., "user prefers morning workouts")
- 0.9-1.0: Core insight about user behavior (e.g., "user procrastinates when overwhelmed")

Return ONLY a JSON array of numbers in the same order as the memories.
Example: [0.3, 0.8, 0.5, 0.9]`;

  try {
    const response = await fetch(`${AZURE_ENDPOINT}/openai/v1/chat/completions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${AZURE_API_KEY}`,
      },
      body: JSON.stringify({
        model: MODEL_NAME,
        messages: [{ role: 'user', content: prompt }],
        max_completion_tokens: 200,
        temperature: 0.1,
      }),
    });

    if (!response.ok) {
      console.error('Importance evaluation API failed:', response.status);
      return new Map(memories.map(m => [m.id, 0.5]));
    }

    const result = await response.json();
    const content = result.choices?.[0]?.message?.content || '[]';

    // 尝试解析 JSON
    let scores: number[];
    try {
      scores = JSON.parse(content.trim());
    } catch {
      // 尝试从文本中提取数字
      const matches = content.match(/[\d.]+/g);
      scores = matches ? matches.map(Number) : [];
    }

    const scoreMap = new Map<string, number>();
    memories.forEach((m, i) => {
      const score = scores[i];
      scoreMap.set(m.id, (typeof score === 'number' && !isNaN(score)) ? score : 0.5);
    });

    return scoreMap;
  } catch (error) {
    console.error('Importance evaluation failed:', error);
    return new Map(memories.map(m => [m.id, 0.5]));
  }
}

/**
 * 检测并解决矛盾记忆
 */
async function resolveContradictions(
  supabase: ReturnType<typeof createClient>,
  userId: string
): Promise<number> {
  // 获取活跃记忆
  const { data: memories, error } = await supabase
    .from('user_memories')
    .select('id, content, tag, created_at, version')
    .eq('user_id', userId)
    .eq('compression_status', 'active')
    .order('created_at', { ascending: false });

  if (error || !memories || memories.length < 2) {
    return 0;
  }

  // 按 tag 分组
  const byTag = new Map<string, typeof memories>();
  for (const m of memories) {
    if (!byTag.has(m.tag)) byTag.set(m.tag, []);
    byTag.get(m.tag)!.push(m);
  }

  let resolved = 0;

  for (const [tag, tagMemories] of byTag) {
    if (tagMemories.length < 2) continue;

    // 使用 AI 检测矛盾
    const prompt = `Check if any of these memories about the same topic contradict each other.

Memories (${tag}):
${tagMemories.slice(0, 10).map((m, i) => `${i + 1}. ${m.content}`).join('\n')}

If there are contradictions (e.g., opposite preferences, changed facts), return:
{
  "hasContradiction": true,
  "pairs": [[olderIndex, newerIndex], ...]
}

Where olderIndex is the index of the OLDER (outdated) memory and newerIndex is the NEWER (current) one.
Memories with higher index numbers are more recent.

If no contradictions found:
{"hasContradiction": false}

Return ONLY the JSON object.`;

    try {
      const response = await fetch(`${AZURE_ENDPOINT}/openai/v1/chat/completions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${AZURE_API_KEY}`,
        },
        body: JSON.stringify({
          model: MODEL_NAME,
          messages: [{ role: 'user', content: prompt }],
          max_completion_tokens: 300,
          temperature: 0.1,
        }),
      });

      if (!response.ok) continue;

      const result = await response.json();
      const content = result.choices?.[0]?.message?.content || '{}';

      let analysis;
      try {
        analysis = JSON.parse(content.trim());
      } catch {
        continue;
      }

      if (analysis.hasContradiction && Array.isArray(analysis.pairs)) {
        for (const pair of analysis.pairs) {
          if (!Array.isArray(pair) || pair.length !== 2) continue;

          const [oldIdx, newIdx] = pair;
          if (oldIdx < 1 || oldIdx > tagMemories.length) continue;
          if (newIdx < 1 || newIdx > tagMemories.length) continue;

          const oldMemory = tagMemories[oldIdx - 1];
          const newMemory = tagMemories[newIdx - 1];

          // 标记旧记忆被替代
          await supabase
            .from('user_memories')
            .update({
              superseded_by: newMemory.id,
              compression_status: 'compressed',
              updated_at: new Date().toISOString(),
            })
            .eq('id', oldMemory.id);

          // 增加新记忆的版本号
          await supabase
            .from('user_memories')
            .update({
              version: (newMemory.version || 1) + 1,
              updated_at: new Date().toISOString(),
            })
            .eq('id', newMemory.id);

          resolved++;
          console.log(`🔄 Resolved contradiction: "${oldMemory.content}" → "${newMemory.content}"`);
        }
      }
    } catch (error) {
      console.error(`Contradiction check failed for tag ${tag}:`, error);
    }
  }

  return resolved;
}

/**
 * 压缩低价值记忆
 */
async function compressLowValueMemories(
  supabase: ReturnType<typeof createClient>,
  userId: string
): Promise<{ evaluated: number; deleted: number; compressed: number }> {
  // 获取压缩候选
  const { data: candidates, error } = await supabase.rpc('get_compression_candidates', {
    p_user_id: userId,
    p_min_age_days: MIN_AGE_DAYS,
    p_low_importance_threshold: LOW_IMPORTANCE_THRESHOLD,
    p_limit: 50,
  });

  if (error || !candidates || candidates.length === 0) {
    return { evaluated: 0, deleted: 0, compressed: 0 };
  }

  console.log(`📋 Found ${candidates.length} compression candidates for user ${userId}`);

  // 重新评估重要性
  const importanceScores = await evaluateImportance(candidates);

  // 分类处理
  const toDelete: string[] = [];
  const toCompress: string[] = [];
  const toUpdate: { id: string; score: number }[] = [];

  for (const candidate of candidates) {
    const score = importanceScores.get(candidate.id) || 0.5;

    if (score < 0.2) {
      toDelete.push(candidate.id);
      console.log(`🗑️ Will delete (score ${score.toFixed(2)}): "${candidate.content.slice(0, 50)}..."`);
    } else if (score < 0.4) {
      toCompress.push(candidate.id);
      console.log(`📦 Will compress (score ${score.toFixed(2)}): "${candidate.content.slice(0, 50)}..."`);
    } else {
      // 更新重要性评分但保留
      toUpdate.push({ id: candidate.id, score });
    }
  }

  // 执行删除
  if (toDelete.length > 0) {
    await supabase.rpc('mark_memories_compressed', {
      p_memory_ids: toDelete,
      p_action: 'delete',
    });
  }

  // 执行压缩（软删除）
  if (toCompress.length > 0) {
    await supabase.rpc('mark_memories_compressed', {
      p_memory_ids: toCompress,
      p_action: 'compress',
    });
  }

  // 更新保留记忆的重要性评分
  for (const { id, score } of toUpdate) {
    await supabase
      .from('user_memories')
      .update({ importance_score: score })
      .eq('id', id);
  }

  return {
    evaluated: candidates.length,
    deleted: toDelete.length,
    compressed: toCompress.length,
  };
}

/**
 * 主处理函数
 */
serve(async (req) => {
  // CORS 预检
  if (req.method === 'OPTIONS') {
    return new Response('ok', { headers: corsHeaders });
  }

  const startTime = Date.now();

  try {
    // 初始化 Supabase
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
    const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;
    const supabase = createClient(supabaseUrl, supabaseServiceKey);

    const { action, userId } = await req.json();
    console.log(`🚀 Memory compressor started: action=${action}, userId=${userId || 'all'}`);

    let result: unknown;

    switch (action) {
      case 'compress_all': {
        // 获取所有有活跃记忆的用户
        const { data: users } = await supabase
          .from('user_memories')
          .select('user_id')
          .eq('compression_status', 'active');

        const uniqueUsers = [...new Set(users?.map(u => u.user_id) || [])];
        console.log(`👥 Processing ${uniqueUsers.length} users`);

        const totals: CompressionResult = {
          evaluated: 0,
          deleted: 0,
          compressed: 0,
          contradictionsResolved: 0,
          mergedGroups: 0,
        };

        for (const uid of uniqueUsers) {
          try {
            const compression = await compressLowValueMemories(supabase, uid);
            const contradictions = await resolveContradictions(supabase, uid);

            totals.evaluated += compression.evaluated;
            totals.deleted += compression.deleted;
            totals.compressed += compression.compressed;
            totals.contradictionsResolved += contradictions;
          } catch (error) {
            console.error(`Error processing user ${uid}:`, error);
          }
        }

        result = {
          success: true,
          usersProcessed: uniqueUsers.length,
          ...totals,
          durationMs: Date.now() - startTime,
        };
        break;
      }

      case 'compress_user': {
        if (!userId) {
          throw new Error('Missing userId for compress_user action');
        }

        const compression = await compressLowValueMemories(supabase, userId);
        const contradictions = await resolveContradictions(supabase, userId);

        result = {
          success: true,
          userId,
          ...compression,
          contradictionsResolved: contradictions,
          durationMs: Date.now() - startTime,
        };
        break;
      }

      case 'dry_run': {
        if (!userId) {
          throw new Error('Missing userId for dry_run action');
        }

        // 只获取候选，不执行
        const { data: candidates } = await supabase.rpc('get_compression_candidates', {
          p_user_id: userId,
          p_min_age_days: MIN_AGE_DAYS,
          p_low_importance_threshold: LOW_IMPORTANCE_THRESHOLD,
          p_limit: 50,
        });

        const importanceScores = await evaluateImportance(candidates || []);

        result = {
          success: true,
          userId,
          candidates: (candidates || []).map(c => ({
            ...c,
            evaluatedScore: importanceScores.get(c.id),
            action: importanceScores.get(c.id)! < 0.2 ? 'delete'
              : importanceScores.get(c.id)! < 0.4 ? 'compress'
              : 'keep',
          })),
          durationMs: Date.now() - startTime,
        };
        break;
      }

      default:
        throw new Error(`Unknown action: ${action}`);
    }

    console.log(`✅ Memory compressor completed in ${Date.now() - startTime}ms`);
    console.log(JSON.stringify(result, null, 2));

    return new Response(
      JSON.stringify(result),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } catch (error) {
    console.error('❌ Memory compressor error:', error);
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message,
        durationMs: Date.now() - startTime,
      }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
});
```

---

## 文件变更清单

| 操作 | 文件路径 | 说明 |
|------|---------|------|
| **新建** | `supabase/migrations/20260127100000_tolan_memory_system.sql` | 数据库迁移 |
| **新建** | `supabase/migrations/20260127110000_memory_compression_cron.sql` | Cron 配置 |
| **新建** | `supabase/functions/memory-compressor/index.ts` | 夜间压缩函数 |
| **新建** | `supabase/functions/_shared/memory-retrieval.ts` | 记忆检索共享模块 |
| **新建** | `supabase/functions/retrieve-memories/index.ts` | 虚拟消息专用记忆检索 API |
| **修改** | `supabase/functions/get-system-instruction/index.ts` | 导入共享模块 + Multi-Query RAG |
| **修改** | `supabase/functions/memory-extractor/index.ts` | 添加重要性评分 |
| **更新** | `docs/architecture/memory-system.md` | 更新架构文档 |

---

## 12. 与动态虚拟消息系统集成

### 12.1 背景

Gemini Live 的 system prompt 只能在会话开始时设置一次，无法中途修改。因此需要通过「虚拟消息」机制在会话过程中动态注入记忆。

详细方案见：`docs/in-progress/20260127-dynamic-virtual-messages.md`

### 12.2 核心思路

将 Tolan 的记忆检索能力抽取为共享模块，供两个场景复用：

| 场景 | 调用时机 | 调用的 API | 输出形式 |
|------|---------|-----------|---------|
| **启动时注入** | AI 会话开始 | `get-system-instruction` | 注入 system prompt |
| **动态注入** | 话题变化时 | `retrieve-memories` | `[CONTEXT]` 虚拟消息 |

### 12.3 共享模块设计

创建 `supabase/functions/_shared/memory-retrieval.ts`，包含以下可复用函数：

```typescript
// 问题合成 - 从用户输入/话题生成检索问题
export async function synthesizeQuestions(
  taskDescription: string,
  recentContext?: string
): Promise<string[]>

// 批量生成 embeddings
export async function generateEmbeddings(
  texts: string[]
): Promise<number[][]>

// MRR 融合排序
export function mergeWithMRR(
  results: MemorySearchResult[]
): MemorySearchResult[]

// 完整的 Multi-Query RAG 检索
export async function multiQueryRAG(
  supabase: SupabaseClient,
  userId: string,
  questions: string[],
  limit?: number
): Promise<FormattedMemory[]>
```

### 12.4 新增 API: `retrieve-memories`

专门供虚拟消息系统调用：

```typescript
// 请求
{
  userId: string;
  currentTopic: string;           // 当前话题
  keywords?: string[];            // 额外关键词
  conversationSummary?: string;   // 对话摘要（更精准检索）
  limit?: number;                 // 返回数量（默认 5）
}

// 响应
{
  memories: Array<{
    content: string;
    tag: string;
    relevance: number;  // MRR 得分
  }>;
  synthesizedQuestions: string[];  // 调试用
  durationMs: number;
}
```

### 12.5 数据流

```
话题检测: "失恋"
        │
        ▼
调用 retrieve-memories API
        │
        ├──► synthesizeQuestions("失恋")
        │    → ["用户之前如何处理失恋？", "情绪低落时什么方法有效？", ...]
        │
        ├──► generateEmbeddings(questions)
        │
        ├──► multiQueryRAG(embeddings)
        │
        └──► MRR 融合排序
        │
        ▼
返回相关记忆 → 生成 [CONTEXT] 虚拟消息 → sendTextMessage
```

---

## 11. 常见问题与踩坑记录

### 11.1 本地 Supabase JWT 验证失败 (ES256/HS256 兼容问题)

**问题描述：**

本地运行 `supabase start` 后，调用 Edge Functions 报错：
```
signing method HS256 is invalid
```

**根本原因：**

Supabase CLI 2.72+ 版本默认使用 ES256 签名算法生成 JWT，但旧的 `anon key` 是用 HS256 签名的，导致 Edge Functions 的 JWT 验证失败。

**参考 Issues：**
- https://github.com/supabase/supabase/issues/42037
- https://github.com/supabase/cli/issues/4098

**临时解决方案：**

将所有需要认证的 Edge Functions 的 `verify_jwt` 设为 `false`：

```toml
# supabase/config.toml

# TODO: 本地开发临时关闭 JWT 验证（Supabase CLI 2.72+ 的 ES256/HS256 兼容问题）
# 生产环境通过 Supabase Dashboard 设置 verify_jwt = true

[functions.get-system-instruction]
verify_jwt = false

[functions.memory-extractor]
verify_jwt = false

[functions.memory-compressor]
verify_jwt = false

[functions.suggest-task-time]
verify_jwt = false

# ... 其他 functions 同理
```

**重要说明：**

| 环境 | 影响 | 安全性 |
|------|------|--------|
| 本地开发 | 需要手动关闭 JWT 验证 | 本地环境无安全风险 |
| 生产环境 | **不受影响** | config.toml 不会部署到生产，生产通过 Dashboard 配置 |

**应用配置后重启：**

```bash
# 停止并重启 Supabase
supabase stop && supabase start

# 重新启动 Edge Functions（加载环境变量）
supabase functions serve --env-file supabase/.env.local
```

---

### 11.2 本地 Supabase HTTPS 证书不被浏览器信任

**问题描述：**

本地启动 Supabase 后，前端调用 Edge Functions 失败，控制台显示网络错误或 CORS 错误。

**原因：**

Supabase 本地服务使用自签名 SSL 证书，浏览器默认不信任，会静默拒绝请求。

**解决方法：**

1. 运行 `supabase status` 查看本地服务地址（API URL 那一行）
2. 在浏览器中直接访问该地址（例如 `https://127.0.0.1:54321`）
3. 浏览器会显示证书警告，点击「高级」→「继续前往」
4. 看到 Supabase 欢迎页面后，说明证书已被信任
5. 返回前端页面刷新，Edge Functions 调用恢复正常

**如何找到本地服务地址：**

```bash
supabase status
```

输出示例：
```
API URL: https://127.0.0.1:54321  ← 访问这个地址
...
```

**注意：** 每次重启电脑或清除浏览器数据后可能需要重新操作。

---

### 11.3 本地登录只能用测试账号

**问题描述：**

本地开发时，点击 Google 或 Apple 登录会报错。

**原因：**

本地 Supabase 没有配置 Google/Apple OAuth 的 Client ID 和 Secret，第三方登录无法使用。

**解决方法：**

使用邮箱密码登录测试账号：

| 邮箱 | 密码 |
|------|------|
| `q@q.com` | （项目约定的测试密码） |

**前提条件：**

本地 Supabase 数据库必须存在这个测试用户。如果是新 clone 的项目或重置过数据库，需要先创建：

```bash
# 方法 1：通过 Supabase Studio 创建
# 访问 http://127.0.0.1:54323 → Authentication → Users → Add user

# 方法 2：通过 SQL 创建（在 Supabase Studio 的 SQL Editor 中执行）
# 具体 SQL 见项目 seed 文件
```

**常见报错：**

- `Invalid login credentials` → 用户不存在或密码错误
- `OAuth error` / `redirect_uri mismatch` → 尝试用了 Google/Apple 登录

---

*文档结束*
