export class AudioStreamer {
  ...
  addPCM16(chunk: Uint8Array) {
    // Don't add audio if context is closed
  ...
  private scheduleNextBuffer() {
    // Don't schedule if context is closed
    if (this.context.state === 'closed') {
  ...
  stop() {
    ...
    // Only modify audio nodes if context is still open
    if (this.context.state !== 'closed') {
      ...
      setTimeout(() => {
        // Double-check context is still open before recreating nodes
        if (this.context.state !== 'closed') {
  ...
  async resume() {
    // Don't resume if context is closed
    if (this.context.state === 'closed') {
      return;
    }
