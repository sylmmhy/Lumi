...
type SuccessScene = 1 | 2 | 3 | 4 | 5;

interface OnboardingCelebrationViewProps {
    completionFlow: CompletionFlow;
    setCompletionFlow: (flow: CompletionFlow) => void;
    successScene: SuccessScene;
    coins: number;
    progressPercent: number;
    showConfetti: boolean;
    completionRemainingTime: number;
    taskDescription: string;
    onSaveAndChallenge: () => void;
    onTakeMoreChallenge: () => void;
}

export function OnboardingCelebrationView({
    completionFlow,
    setCompletionFlow,
    successScene,
    coins,
    ...
    onSaveAndChallenge,
    onTakeMoreChallenge,
}: OnboardingCelebrationViewProps) {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center" style={{ backgroundColor: '#1e1e1e' }}>
            {/* Confirmation Screen */}
            {completionFlow === 'confirm' && (
                <div className="flex flex-col items-center gap-8 px-6 max-w-md">
                    ...
                    <div className="flex flex-col gap-4 w-full">
                        {/* YES Button */}
                        <button
                            onClick={() => setCompletionFlow('success')}
                        ...
                        {/* NO Button */}
                        <button
                            onClick={() => setCompletionFlow('failure')}
            ...
            {/* Success Flow - Scene 1: You Made It! */}
            {completionFlow === 'success' && (
                <div className="fixed inset-0" style={{ backgroundColor: '#1e1e1e' }}>
                    {/* Content - Fixed layout from Scene 1 */}
                    <div className="relative flex flex-col items-center justify-center h-full px-6">
                        <div className="flex flex-col items-center gap-8 w-full max-w-md">
                            ...
                            >
                                ...
                                >
                                    {formatTime(completionRemainingTime)}
                                </span>
                            </div>
                            ...
                            {/* LEVEL:1 + Progress Bar + Crown - Always rendered from Scene 1 */}
                            <div
                                ...
                                style={{
                                    ...
                                    borderRadius: '16px',
                                    backgroundColor: '#2E2B28',
                                    visibility: successScene >= 2 ? 'visible' : 'hidden',
                                    animation: successScene >= 2 ? 'slideUpFadeIn 0.8s ease-out forwards' : 'none',
                                    opacity: successScene >= 2 ? 0 : 0
                                }}
                            >
                                <div className="flex flex-col gap-2" style={{ width: '249px' }}>
                                    ...
                                    >
                                        ...
                                        />
                                        {/* Shimmer effect (only visible when filling) */}
                                        {successScene === 3 && progressPercent < 80 && (
                                            <div
                                                style={{
                                                    position: 'absolute',
                                                    top: 0,
                            ...
                            {/* Coins and CTA Button Container - Same position */}
                            <div style={{ position: 'relative', width: '323px', height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                {/* +500 Coins Display - Scene 2-3 */}
                                <div
                                    className="flex items-center gap-3"
                                    style={{
                                        position: 'absolute',
                                        visibility: successScene >= 2 && successScene < 4 ? 'visible' : 'hidden',
                                        animation: successScene >= 2 ? 'slideUpFadeIn 0.8s ease-out 0.2s forwards' : 'none',
                                        opacity: successScene >= 2 ? 0 : 0
                                    }}
                                >
                                    <span
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        animation: successScene >= 4 ? 'slideUpBounce 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards' : 'none',
                                        opacity: 0,
                                        visibility: successScene >= 4 ? 'visible' : 'hidden',
                                        pointerEvents: successScene >= 4 ? 'auto' : 'none'
                                    }}
                                >
                                    <span
                                        className="font-bold uppercase"
            ...
            {/* Failure Flow - Encouragement Screen */}
            {completionFlow === 'failure' && (
                <div className="fixed inset-0" style={{ backgroundColor: '#1e1e1e' }}>
                    <div className="relative flex flex-col items-center justify-center h-full px-6 gap-12">
                        ...
                        >
                            You Can Make It<br />Next Time!
                        </div>

                        {/* Take more challenge Button - Same style as Success CTA */}
                        <button
            ...
            {completionFlow === 'success' && (
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 9999, pointerEvents: 'none' }}>
                    <Confetti
                        ...
                        numberOfPieces={showConfetti ? 5000 : 0}
