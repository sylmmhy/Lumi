...
export async function transcribeAudio(audioFile, abortSignal) {
  const config = resolveTranscriptionProvider();
  if (!config.apiKey) {
    const missingKey = config.provider === "azure" ? AZURE_CONFIG.apiKey : OPENAI_CONFIG.apiKey;
    throw new Error(`${config.provider === "azure" ? "Azure" : "OpenAI"} API key not configured (${missingKey})`);
  }
  if (!config.url) {
    const missingUrl = config.provider === "azure" ? AZURE_CONFIG.endpoint : OPENAI_CONFIG.endpoint;
    throw new Error(`${config.provider === "azure" ? "Azure" : "OpenAI"} transcription endpoint not configured (${missingUrl})`);
  }
  if (!config.model) {
    const missingModel = config.provider === "azure" ? AZURE_CONFIG.model : OPENAI_CONFIG.model;
    throw new Error(`${config.provider === "azure" ? "Azure" : "OpenAI"} transcription model not configured (${missingModel})`);
  }
