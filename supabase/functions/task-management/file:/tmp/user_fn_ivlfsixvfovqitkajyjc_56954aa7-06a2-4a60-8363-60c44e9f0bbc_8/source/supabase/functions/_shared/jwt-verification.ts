...
async function verifyJWTWithJose(token) {
  try {
...
export async function verifyJWTToken(authHeader) {
  try {
    // Extract token
    ...
    const { data: user, error } = await supabase.from('users').select('id, email, name, google_id').eq('id', payload.user_id).not('google_id', 'is', null) // Ensure it's a Google user
    .single();
    if (error || !user) {
      return {
        valid: false,
